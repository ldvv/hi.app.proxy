...
public class ToolsService {
    ...
    public boolean editAndCreate(String jspFile, String dicPath, String resultPath, String propertyFile) {
        logger.info("D9999: Begin operate jsp file------------->" + jspFile.substring(jspFile.lastIndexOf("\\"), jspFile.length()));

        StringBuffer content = new StringBuffer("");

        Pattern patternNotes = Pattern.compile("crm.global.\\d{5}");
        ...
        try {
            ...
            /*******************替换Key*************************/
            logger.info("D9999: Replace Keys!");
            Properties pro = new Properties();
            in = new FileInputStream(propertyFile);
            pro.load(in);
            ...
            Matcher matcherNotes = patternNotes.matcher(contentStr);
            while(matcherNotes.find()){
                String key = contentStr.substring(matcherNotes.start(), matcherNotes.end());
                if(pro.getProperty(key) == null){
                    logger.error("D9999: #######################" + key);
                }else {
                    contentStr = contentStr.replaceAll(key, pro.getProperty(key));
                }
                matcherNotes = patternNotes.matcher(contentStr);
            }
